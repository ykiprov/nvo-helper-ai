...
import { GraduationCap, MessageCircle, BookOpen, Settings, LogIn, Target } from "lucide-react";
...
type Tab = "home" | "chat" | "materials" | "tests" | "admin";
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1">
        {activeTab === "home" && <HeroSection onStartChat={() => setActiveTab("chat")} onMaterials={() => setActiveTab("materials")} onTests={() => setActiveTab("tests")} />}
...
function HeroSection({ onStartChat, onMaterials, onTests }: { onStartChat: () => void; onMaterials: () => void; onTests: () => void }) {
  return (
    <div className="max-w-3xl mx-auto px-4 py-16 text-center">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          <button onClick={onStartChat} className="gradient-primary text-primary-foreground font-semibold px-8 py-3 rounded-xl hover:opacity-90 transition-opacity text-sm">💬 Започни чат</button>
          <button onClick={onTests} className="bg-card border border-border text-foreground font-semibold px-8 py-3 rounded-xl hover:bg-muted transition-colors text-sm">🎯 Реши тест</button>
          <button onClick={onMaterials} className="bg-card border border-border text-foreground font-semibold px-8 py-3 rounded-xl hover:bg-muted transition-colors text-sm">📚 Виж материали</button>
        </div>

        <div className="mt-16 grid grid-cols-1 sm:grid-cols-4 gap-4">
          {[
            { emoji: "📝", title: "Пробна матура", desc: "Реалистична НВО симулация" },
            { emoji: "📐", title: "Математика", desc: "Алгебра, геометрия, задачи" },
            { emoji: "🇧🇬", title: "БЕЛ", desc: "Граматика, правопис, литература" },
